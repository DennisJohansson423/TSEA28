...
;;
;;                 student LiU-ID: ______Denjo163___________
;; + lab group participant LiU-ID: ________Anthu456_________
...
intgpiod:
    ; BCD
    ; Nollställ avbrottsflaggan
    mov r0, #(GPIOD_GPIOICR & 0xffff)
    movt r0, #(GPIOD_GPIOICR >> 16)
    mov r1, #(1 << 7)
    str r1, [r0]

    ; Ladda in positionen på nästa siffra
    mov r12, #(0x20001004 & 0xffff)
    movt r12, #(0x20001004 >> 16)
    ldrb r1, [r12]

    ; Skicka positionen till räknarens display
    mov r0, #(GPIOF_GPIODATA & 0xffff)
    movt r0, #(GPIOF_GPIODATA >> 16)
    str r1, [r0]

    ; Tar fram värdet på nästkommande siffra som ska skrivas ut
    mov r0, #(0x20001000 & 0xffff)
    movt r0, #(0x20001000 >> 16)
    add r0, r0, r1
    ldrb r0, [r0]

    AND r3, r0, #0x0F ; Maska ut siffran (fyra bitar),
                      ; antar att den finns i R0 här
    ADR r2, SJUSEGTAB ; Tabellstart till R2
    ADD r2, r2, r3 ; Peka ut r¨att byte
    LDRB r3, [r2] ; Hämta bitmönstret

    ; Skicka det framtagna värdet på siffran till displayen
    mov r5, #(GPIOB_GPIODATA & 0xffff)
    movt r5, #(GPIOB_GPIODATA >> 16)
    str r3, [r5]

    ; Räkna upp positionen med ett eller återställ till noll,
    ; beroende på vad räknaren står på
    add r1, r1, #1
    cmp r1, #4
    bge resetPos
...
intgpioe:
    ...
    ; Kontrollera om sekundvisaren har värdet 9, 
    ; då ska den slå om till noll, annars +1
    ldrb r1, [r0]
    cmp r1, #9
    beq addten
...
addten: 
    ; +1 till 10-sekundvisaren,
    ; sätter sekundvisaren till noll
    mov r1, #0
    strb r1, [r0]

    ldrb r1, [r0, #1]

    ; kontrollera om minutvisaren är 5, 
    ; för då ska den börja om, annars +1
    cmp r1, #5
    beq addmin
...
resetmin: 
    ; Återställ när klockan nått 60 minuter
    mov r1, #0
    strb r1, [r0, #3]
